 ...
 *
 * Both shared and private maps are stored in single-dimentional array. Private
 * event entries, however, are effectively 2D array.
 */
sdei_entry_t *get_event_entry(sdei_ev_map_t *map)
{
	...
	if (is_event_private(map)) {
		...
		/*
		 * Return the addresss of the entry at the same index in the
		 * per-CPU event entry.
		 */
